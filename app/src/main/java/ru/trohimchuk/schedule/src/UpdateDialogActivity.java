...
import ru.trohimchuk.schedule.utils.MySubjectUpdateManager;
import ru.trohimchuk.schedule.utils.StringConstants;
import ru.trohimchuk.schedule.utils.SubjectToRemove;
import ru.trohimchuk.schedule.utils.XMLParser;
	...
	public void onClick(View v) {
		...
		case R.id.update_load:
			AsyncUpdater updater = new AsyncUpdater(getSharedPreferences(
					StringConstants.SCHEDULE_SHARED_PREFERENCES, MODE_PRIVATE)
					.getString(StringConstants.TOKEN, null));
			updater.execute((Void[]) null); // fix, don't pass NULL here and other places, use an array.
			break;
